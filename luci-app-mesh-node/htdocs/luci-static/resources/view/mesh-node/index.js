...
function m11opt(s, widget, name, section, title, desc) {
	...
	o.depends('mesh_node.main.m11_enable', '1');
...
return view.extend({
	...
	render: function ([info, running]) {
		...
		s.tab('mesh11sd',      _('Mesh11sd Daemon'));
		...
		o = s.taboption('mesh', form.ListValue, 'batadv_algo', _('batman-adv Routing Algorithm'),
		...
		o.onclick = function () {
			if (uci.get_bool('mesh_node', 'main', 'use_batadv')) {
				...
				return Promise.all([
					run(['meshif', 'bat0', 'interface']),           // 1 底层接口
					run(['meshif', 'bat0', 'statistics']),          // 2 统计
					run(['meshif', 'bat0', 'neighbors']),           // 3 邻居
					run(['meshif', 'bat0', 'originators']),         // 4 全网路由表
					run(['meshif', 'bat0', 'translocal']),          // 5 本地转发表
					run(['meshif', 'bat0', 'transglobal'])          // 6 全局转发表
				]).then(function (r) {
					var sep = '─'.repeat(72);
					var text = '';
					text += `▶ ${_('BACKHAUL INTERFACES')}  ${_('(meshif bat0 interface)')}   \n${sep}\n${r[1] || _('none')}\n\n`;
					text += `▶ ${_('STATISTICS')}           ${_('(meshif bat0 statistics)')}  \n${sep}\n${r[2] || _('none')}\n\n`;
					text += `▶ ${_('NEIGHBORS')}            ${_('(meshif bat0 neighbors)')}   \n${sep}\n${r[3] || _('none')}\n\n`;
					text += `▶ ${_('ORIGINATORS')}          ${_('(meshif bat0 originators)')} \n${sep}\n${r[4] || _('none')}\n\n`;
					text += `▶ ${_('TRANSLATION LOCAL')}    ${_('(meshif bat0 translocal)')}  \n${sep}\n${r[5] || _('none')}\n\n`;
					text += `▶ ${_('TRANSLATION GLOBAL')}   ${_('(meshif bat0 transglobal)')} \n${sep}\n${r[6] || _('none')}\n`;
		...
		// ══ Mesh11sd Daemon tab ═══════════════════════════════════════════
		o = s.taboption('mesh11sd', form.DummyValue, '_m11_badge', _('Daemon Status'));
		o.rawhtml = true; o.depends('use_batadv', '0');
		o.cfgvalue = function () {
		...
		o = s.taboption('mesh11sd', form.Flag, 'm11_enable', _('Enable mesh11sd Daemon'),
		...
		o.default = '0'; o.rmempty = false; o.depends('use_batadv', '0');

		o = m11opt(s, form.Value, 'checkinterval', null, _('Check Interval (seconds)'),
			_('How often the daemon checks and updates mesh configuration. Minimum: 10.'));
		o.datatype = 'min(10)'; o.default = '10';

		o = m11opt(s, form.ListValue, 'debuglevel', null, _('Log Level'),
		...
		o = m11opt(s, form.ListValue, 'portal_detect', null, _('Portal Detection Mode'),
		...
		o = m11opt(s, form.Value, 'portal_channel', null, _('Portal Channel (2.4 GHz)'),
		...
		o = m11opt(s, form.Flag, 'portal_use_default_ipv4', null, _('Use Default IPv4'),
		...
		o = m11opt(s, form.Value, 'portal_detect_threshold', null, _('Portal Detect Threshold'),
		...
		o = m11opt(s, form.Value, 'channel_tracking_checkinterval', null, _('Channel Tracking Checkinterval (seconds)'),
		...
		o = m11opt(s, form.Value, 'auto_mesh_id', null, _('Auto Mesh ID'),
		...
		o = m11opt(s, form.ListValue, 'auto_mesh_band', null, _('Auto Mesh Band'),
		...
		o = m11opt(s, form.Value, 'auto_mesh_key', null, _('Auto Mesh Key'),
		...
		o = m11opt(s, form.Value, 'mesh_phy_index', null, _('Mesh PHY Index'),
		...
		o = m11opt(s, form.Value, 'country', null, _('Country Code'),
		...
		o = m11opt(s, form.Value, 'mesh_basename', null, _('Mesh Basename'),
		...
		o = m11opt(s, form.Value, 'mesh_gate_base_ssid', null, _('Mesh Gate Base SSID'),
		...
		o = m11opt(s, form.ListValue, 'mesh_gate_encryption', null, _('Mesh Gate Encryption'),
		...
		o = m11opt(s, form.Value, 'mesh_gate_key', null, _('Mesh Gate Key'),
		...
		o = m11opt(s, form.Flag, 'ssid_suffix_enable', null, _('SSID Suffix Enable'),
		...
		o = m11opt(s, form.Flag, 'vtun_enable', null, _('VXLAN Tunnel Enable'),
		...
		o = m11opt(s, form.Value, 'tun_id', null, _('Tunnel ID'),
		...
		o = m11opt(s, form.Value, 'vtun_ip', null, _('VXLAN Tunnel IPv4 Gateway'),
		...
		o = m11opt(s, form.Value, 'vtun_mask', null, _('VXLAN Tunnel IPv4 Mask'),
			_('Subnet mask for the VXLAN tunnel address.'));
		o.datatype = 'ip4addr'; o.placeholder = '255.255.255.0';
		o.deps = []; o.depends({ vtun_enable: '1', portal_detect: /^(0|1)$/ });

		o = m11opt(s, form.RichListValue, 'vtun_gate_encryption', null, _('VXLAN Tunnel Gate Encryption'),
		...
		o = m11opt(s, form.Value, 'vtun_gate_key', null, _('VXLAN Tunnel Gate Key'),
		...
		o = m11opt(s, form.Value, 'vtun_base_ssid', null, _('VXLAN Tunnel Base SSID'),
			_('Base SSID for access points connected to the VXLAN tunnel. ' +
			  'Maximum 30 characters (or 22 if SSID Suffix is enabled).'));
		o.datatype = 'maxlength(30)'; o.default = 'Guest';
		o.deps = []; o.depends('vtun_enable', '1');

		o = m11opt(s, form.Value, 'vtun_path_cost', null, _('VXLAN Tunnel Path Cost'),
			_('STP cost assigned to the VXLAN tunnel network. Range: 0–65534. Set to 0 to disable STP.'));
		o.datatype = 'range(0,65534)'; o.default = '10';
		o.deps = []; o.depends('vtun_enable', '1');

		o = m11opt(s, form.RichListValue, 'mesh_gate_enable', null, _('Mesh Gate Enable'),
		...
		o = m11opt(s, form.Flag, 'mesh_leechmode_enable', null, _('Leech Mode Enable'),
		...
		o = m11opt(s, form.Value, 'txpower', null, _('TX Power (dBm)'),
		...
		o = m11opt(s, form.Flag, 'mesh_path_stabilisation', null, _('Mesh Path Stabilisation'),
			_('Prevents frequent multi-hop path changes caused by multipath signal strength jitter.'));
		o.default = '1';

		o = m11opt(s, form.Value, 'reactive_path_stabilisation_threshold', null, _('Reactive Path Stabilisation Threshold'),
			_('Number of consecutive check intervals with an unstable neighbour path before path stabilisation is activated.'));
		o.datatype = 'uinteger'; o.default = '10';

		o = m11opt(s, form.Flag, 'mesh_mac_forced_forwarding', null, _('MAC Forced Forwarding'),
			_('Enables MAC forced forwarding on the mesh interface to prevent ARP flooding.'));
		o.default = '1';

		o = m11opt(s, form.Flag, 'gateway_proxy_arp', null, _('Gateway Proxy ARP'),
			_('Enables proxy ARP on the gateway bridge interface.'));
		o.default = '1';

		o = m11opt(s, form.Flag, 'stop_on_error', null, _('Stop on Error'),
		...
		o = m11opt(s, form.Flag, 'reboot_on_error', null, _('Reboot on Error'),
		...
		o = m11opt(s, form.Flag, 'apmond_enable', null, _('AP Monitor Enable'),
			_('Enables the access point monitoring daemon (apmond). ' +
			  'Collects AP data and forwards it to the portal node.'));
		o.default = '0';

		o = m11opt(s, form.Value, 'apmond_cgi_dir', null, _('AP Monitor CGI Directory'),
		...
		o = m11opt(s, form.Value, 'mesh_backhaul_led', null, _('Mesh Backhaul LED'),
		...
		o = m11opt(s, form.Button, '_mesh11sd_status_btn', null, _('Live Status'));
		...
		o = m11opt(s, form.Button, '_mesh11sd_log_btn', null, _('Daemon Log'));
