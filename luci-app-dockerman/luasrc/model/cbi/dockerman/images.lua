...
local dk = docker.new()
local m, s, o, containers, images, res, lost_state
...
o.write = function(self, section, value)
	pull_value["_image_tag_name"] = value .. (value:find(":") and '' or ':latest')
end
...
o.write = function(self, section)
	...
	if tag and tag ~= "" then
		docker:write_status("Images: pulling %s...\n" %tag)
		...
		if res
			...
			and (res.body[#res.body]
				...
				and (res.body[#res.body].status == "Status: Downloaded newer image for %s" %tag)) then
					docker:clear_status()
		else
			docker:append_status("code:%s %s\n" %{res.code, (res.body[#res.body] and res.body[#res.body].error or (res.body.message or res.message))})
		end
	else
...
local remove_action = function(force)
	...
	if next(image_selected) ~= nil then
		...
		for _, img in ipairs(image_selected) do
			local query
			docker:append_status("Images: remove %s..." %img)
...
o.write = function (self, section)
	...
	if next(image_selected) ~= nil then
		...
		show_name = #image_selected > 1 and "images" or image_selected[1]
		local cb = function(res, chunk)
			if res and res.code and res.code == 200 then
				if not first then
					first = true
					luci.http.header('Content-Disposition', 'inline; filename="%s.tar"' %show_name)
		...
		docker:write_status("Images: save %s..." %table.concat(image_selected, "\n"))
