...
function renderNodeSettings(section) {
    ...
    o = s.option(form.Value, 'tls_utls', _('uTLS fingerprint'),
...
function finalizeConfig(config) {
    ...
    config.address = String(config.address).replace(/\[|\]/g, '');
    config.port = String(config.port);
    return config;
}

function splitWsEarlyData(config, path) {
    if (path && path.includes('?ed=')) {
        config.websocket_early_data_header = 'Sec-WebSocket-Protocol';
        config.websocket_early_data = path.split('?ed=')[1];
        config.ws_path = path.split('?ed=')[0];
    } else {
        config.ws_path = path;
    }
}
        ...
        case 'trojan': {
            ...
            switch (params.get('type')) {
                ...
                case 'ws':
                    ...
                    splitWsEarlyData(config, params.get('path') ? decodeURIComponent(params.get('path')) : null);
                    break;
            }
            break;
        }
        ...
        case 'vmess': {
            ...
            switch (vmess.net) {
                ...
                case 'ws':
                    ...
                    splitWsEarlyData(config, vmess.path);
                    break;
            }
            break;
        }
...
function parseNode(input) {
    ...
    if (/^[a-zA-Z0-9]+:\/\//.test(line)) return parseShareLink(line);
...
function parseFlowYAML(str) {
    let s = str.trim();
    if (s.startsWith('-')) s = s.slice(1).trim();
    const jsonText = flowToJSON(s);
...
return view.extend({
    ...
    load: function () {
        return Promise.all([
            L.resolveDefault(fs.read_direct('/etc/nikki/res/sub_ini.list'), ''),
            uci.load('nikki')
        ]);
    },

    render: function ([iniContent]) {
        let m, s, o, so;
        const self = this;
        ...
        o.value('subconverter_public', _('SubConverter (public)'));
        ...
        const converterFlags = [
            ...
            ['converter_udp',         'udp',         _('UDP Support'),        _('udp=true')],
            ['converter_tfo',         'tfo',         _('TCP Fast Open'),      _('tfo=true')],
            ['converter_tls13',       'tls13',       _('TLS 1.3'),            _('tls13=true')],
            ...
            ['converter_expand',      'expand',      _('Expand'),             _('expand rulesets')],
            ['converter_new_name',    'new_name',    _('Rename Nodes'),       _('new_name=true')],
            ...
            ['converter_classic',     'classic',     _('Classic Rules'),      _('classic rule format')],
            ['converter_list',        'list',        _('Node List Only'),     _('list=true, no groups')],
            ['converter_info_node',   'info',        _('Info Node'),          _('append subscription info as node')],
        ];

        for (const [name, param, title, desc] of converterFlags) {
            o = s.option(form.Flag, name, title, _('%s').format(desc));
            o.modalonly = true; o.rmempty = true; o.default = '0';
            o.depends('use_converter', '1');
        }
        ...
        so.handleLinkImport = function () {
            const textarea = E('textarea', {
                ...
                'placeholder':
                    ...
                    "{ name: '[hy2]香港 2 直连', server: jptv02.outleft-hy.xyz, port: 5002, sni: jptv02.outleft-hy.xyz, up: 700, down: 700, skip-cert-verify: false, type: hysteria2, password: fe19b600-2979-3689-9815-cb0ec465c0ad, obfs: salamander, obfs-password: YTRkMDE1MzQ4NmFlNmQxng== }\n"
            });

            ui.showModal(_('Import share links'), [
                E('p', _('Support Hysteria, Shadowsocks, Trojan, v2rayN (VMess), XTLS (VLESS) share links, and single-line Clash proxy entries.')),
                textarea,
