...
if (ub(uci.get('nikki', 'config', 'url_enabled'))) {
	...
	uci.foreach('nikki', 'subscription', (s) => {
		...
		const name = s.name || `provider${idx}`;
		if (k) return push(exprs,
		...
		push(exprs,
			...
			`${np}.url = ${qs(s.url)}`,
			`${np}.filter = ${qs(filter)}`,
			`${np}.health-check.enable = true`,
			`${np}.health-check.interval = ${hc_int}`,
			`${np}.path = ${qs(`./proxies/${name}.yaml`)}`,
			`${np}.health-check.url = ${qs(hc_url)}`
		);
	});
};
...
if (match(core, /smart/)) {
	...
	if (lgbm) {
		push(exprs,
			...
			`.profile.smart-collector-size = ${ui(collector_size)}`
		);
	};
	...
	for (let i = 0; i < length(groups); i++) {
		...
		if (g && (g.type == 'url-test' || g.type == 'load-balance')) {
			const p = `.proxy-groups[${i}]`;
			push(exprs,
				`${p}.type = "smart"`,
				`${p}.uselightgbm = ${uselightgbm}`,
				`${p}.sample-rate = ${sample_rate}`,
				`${p}.prefer-asn  = ${ub(prefer_asn)}`,
				`${p}.collectdata = ${ub(collectdata)}`,
				`${p}.strategy    = ${qs(smart_strategy)}`,
				...(policy_priority ? [`${p}["policy-priority"] = ${qs(policy_priority)}`] : [])
			);
		}
	}
};
