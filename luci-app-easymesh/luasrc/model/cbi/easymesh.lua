...
local function get_bat_nodes()
	local nodes = {}
	local output = util.exec("batctl n 2>/dev/null | awk 'NR>2 {print $1,$2,$3}'") or ""

	for line in output:gmatch("[^\r\n]+") do
		local iface, neighbor, lastseen = line:match("^(%S+)%s+(%S+)%s+(%S+)")
		if iface then
			nodes[#nodes + 1] = {
				IF = iface,
				Neighbor = neighbor,
...
enable.default = 1
enable.rmempty = false
enable.description = translate("Enable 802.11k/v/r for improved roaming and network management. 802.11k provides neighbor reports, 802.11v supports load balancing, and 802.11r enables fast AP transitions.")
