...
local am_state=(SYS.call("pidof amuled > /dev/null") == 0)
-- local am_state=(SYS.call("[ `(ps | grep amule) 2>/dev/null | grep -c opt 2>/dev/null` -ne 0 ] >/dev/null") == 0)
...
o = s:option(Flag, "qb_boot", translate("开机运行"), translate("开机启动qBittorrent"))
o:depends("qb_enable",1)
local qb_pstate=(SYS.call("[ ! -s /opt/etc/init.d/S89qbittorrent 2>/dev/null ]") == 0)
local qb_state=(SYS.call("[ `(ps | grep qbittorrent-nox) 2>/dev/null | grep -c opt 2>/dev/null` -gt 0 ] >/dev/null") == 0)
if qb_state then
	...
	p.description = translate("qBittorrent默认WebUI端口: 9080，用启名: admin，密码: adminadmin<br>" .. font_green .. "qBittorrent 运行中" .. font_off .. "<br>" .. font_apply .. "打开WebUI管理" .. font_op .. "9080')\"/>" )
	p:depends("qb_enable",1)
else
	if qb_pstate then
...
local rT_state=(SYS.call("[ `(ps | grep rtorrent) 2>/dev/null | grep -c opt 2>/dev/null` -gt 0 ] >/dev/null") == 0)
if rT_state then
...
o = s:option(Flag, "tr_boot", translate("开机运行"), translate("开机启动Transmission"))
o:depends("tr_enable",1)
local tr_pstate=(SYS.call("[ ! -s /opt/etc/init.d/S88transmission 2>/dev/null ]") == 0)
local tr_state=(SYS.call("[ `(ps | grep transmission) 2>/dev/null | grep -c opt 2>/dev/null` -gt 0 ] >/dev/null") == 0)
if tr_state then
	...
	p.description = translate("Transmission默认WebU 用启名: admin，密码: admin<br>当前状态</b>：" .. font_green .. "运行中&nbsp;&nbsp;&nbsp;" .. font_off .. font_apply .. "打开WebUI管理" .. font_op .. "9091')\"/>" )
	p:depends("tr_enable",1)
else
	if tr_pstate then
