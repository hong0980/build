...
return view.extend({
	load: function() {
		return Promise.all([
			L.resolveDefault(fs.exec('/usr/bin/pgrep', ['transmission-daemon']), null)
				.then(r => r.code == 0),
	...
	render: function([running, isweb, diskList]) {
		let m, s, o;
		var port = uci.get_first('transmission', 'transmission', 'rpc_port') || '9091';
		var webinstalled = isweb || !!uci.get_first('transmission', 'transmission', 'web_home');
