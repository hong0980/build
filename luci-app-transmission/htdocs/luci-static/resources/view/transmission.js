...
return view.extend({
	load: function() {
		return Promise.all([
			...
			L.resolveDefault(fs.exec_direct('/bin/df', ['-h'])).then(function(disk) {
	...
	render: function(res) {
		...
		var button = '';
		console.log(running, webinstalled)
		if (running && webinstalled)
			button = '&#160;<a class="btn cbi-button-apply" href="http://' + window.location.hostname + ':' + port + '" target="_blank" rel="noreferrer noopener">' + _('Open Web Interface') + '</a>';
