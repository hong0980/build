...
return view.extend({
	...
	render: ([data, isRunning]) => {
		...
		o.optional = false;
		o.default = 'br-lan';
		o.filter = (d, v) => !/^(@|docker0|veth|teq|br-[0-9a-f]+)/.test(v);
		...
		L.sortedKeys(hosts).forEach(mac => {
		...
		o.validate = (d, v) => {
			if (v === '') return true;
			var m = String(v).toLowerCase().match(/^(?:0x[0-9a-f]{1,8}|[0-9]{1,10})\/([a-z]+)$/);
			return m && ['second', 'minute', 'hour', 'day'].some(u => u.startsWith(m[1]))
				? true
				: _('Invalid limit value');
		};
		...
		o.value('Sat', _('sat'));
		o.value('Sun', _('sun'));
		o.write = function (d, v) {
			return this.super('write', [d, L.toArray(v).join(' ')]);
		};
		...
		o.placeholder = '08:00';
		o.editable = true;
		o.validate = (d, v) => !v || /^([01]\d|2[0-3]):([0-5]\d)$/.test(v) || _('Invalid time format. Use HH:MM.');
